  ...
  return (
    <section className="bg-[#0d0d0d] border-t border-b border-phantom-border-subtle py-8" ref={ref}>
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex items-center justify-center gap-x-8 xl:gap-x-12">
          {STATS.map(({ number, label }, i) => (
            ...
            >
              <div className="text-center">
                <p className="font-code font-bold text-[24px] xl:text-[28px] text-phantom-lime leading-none mb-1">{number}</p>
                <p className="font-body text-[11px] xl:text-[13px] text-phantom-text-muted whitespace-nowrap">{label}</p>
              </div>
              {i < STATS.length - 1 && (
                <div className="w-px h-10 xl:h-12 bg-[#1e1e1e]" />
              )}
            </motion.div>
          ))}
        </div>

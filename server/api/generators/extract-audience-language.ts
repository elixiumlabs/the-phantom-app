...
Return JSON: { "problemPhrases": string[], "emotionalDescriptors": string[], "failedAttemptPhrases": string[], "outcomePhrases": string[], "jargonToAvoid": string[], "examples": [{ "verbatim": string, "whereSaid": string }] }`,
  persist: async (ctx, output) => {
    ...
    if (ctx.project) {
      written.push(await appendGenerationHistory({ project_id: ctx.project.id, generator: 'extractAudienceLanguage', input: ctx.input, output }))
      const db = adminClient()
      await db.from('audience_language').upsert({ project_id: ctx.project.id, ...output, generated_at: new Date().toISOString() }, { onConflict: 'project_id' })
      written.push(`audience_language/${ctx.project.id}`)
    }

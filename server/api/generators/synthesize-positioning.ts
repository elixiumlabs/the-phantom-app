...
interface Out extends Record<string, unknown> {
  positioningOptions: Array<{ sentence: string; angle: 'problem-led' | 'outcome-led' | 'identity-led'; reasoning: string }>
...
Return JSON: { "positioningOptions": [{ "sentence": string, "angle": "problem-led"|"outcome-led"|"identity-led", "reasoning": string }], "workingNames": [{ "name": string, "rationale": string }], "voiceTriples": [{ "adjectives": [string, string, string], "whyItFits": string }] }`,
  persist: async (ctx, output) => {
    ...
    if (ctx.project) {
      written.push(await appendGenerationHistory({ project_id: ctx.project.id, generator: 'synthesizePositioning', input: ctx.input, output }))
      const db = adminClient()
      await db.from('ghost_identity').update({
      ...
      written.push(`ghost_identity/${ctx.project.id}`)
    }

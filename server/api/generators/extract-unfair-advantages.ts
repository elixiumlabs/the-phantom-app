...
Sort advantages by credibilityScore descending.`,
  persist: async (ctx, output) => {
    ...
    if (ctx.project) {
      written.push(await appendGenerationHistory({ project_id: ctx.project.id, generator: 'extractUnfairAdvantages', input: ctx.input, output }))
      const db = adminClient()
      await db.from('ghost_identity').update({
        ai_advantage_options: output.advantages,
        ai_rejected_claims: output.rejected,
        ai_advantages_generated_at: new Date().toISOString(),

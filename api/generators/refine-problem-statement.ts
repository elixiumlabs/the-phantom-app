...
Return JSON: { "refined": [{ "statement": string, "tightened": "specificity"|"audience"|"outcome"|"avoidance", "note": string }] }`,
  persist: async (ctx, output) => {
    ...
    if (ctx.project) {
      written.push(await appendGenerationHistory({ project_id: ctx.project.id, generator: 'refineProblemStatement', input: ctx.input, output }))
      const db = adminClient()
      await db.from('ghost_identity').update({
        ai_problem_options: output.refined,
        ai_problem_generated_at: new Date().toISOString(),
